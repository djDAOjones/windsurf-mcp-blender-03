...
class BlenderMCPServer:
    ...
    def get_viewport_screenshot(self, max_size=800, filepath=None, format="png"):
        ...
        try:
            ...
            # Determine viewport window region size (avoid loading image to prevent crashes)
            width = height = None
            ...
            # Fallback to probing the saved image only for dimensions if region not found
            if width is None or height is None:
                try:
                    tmp = bpy.data.images.load(filepath)
                    width, height = map(int, tmp.size)
                    bpy.data.images.remove(tmp)
                except Exception:
                    width, height = 0, 0
            
            # Note: We intentionally skip resizing here to avoid instability in some Blender builds.
            # If a smaller image is needed, resize it outside Blender after retrieval.
            return {
                "success": True,
                "width": width,
                "height": height,
